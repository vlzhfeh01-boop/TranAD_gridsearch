...
def config_valid(config):
    ...
    try:
        ...
        assert (
            "anneal_function" in keys
            and type(config["anneal_function"]) == str
            and config["anneal_function"] in ["logistic", "linear"]
        )
        ...
        assert "latent_label_weight" in keys and isinstance(
            config["latent_label_weight"], (int, float)
        )
        ...
        assert (
            "model_type" in keys
            and type(config["model_type"]) == str
            and config["model_type"] in ["rnn", "transformer"]
        )
        ...
        assert (
            "rnn_type" in keys
            and type(config["rnn_type"]) == str
            and config["rnn_type"] in ["rnn", "lstm", "gru"]
        )
        ...
        assert (
            "use_flag" in keys
            and type(config["use_flag"]) == str
            and config["use_flag"] in ["rec_error", "l2norm", "copod_score"]
        )
        ...
        assert (
            "num_granularity_all" in keys and type(config["num_granularity_all"]) == int
        )
        assert "granularity_car" in keys and type(config["granularity_car"]) == int
        assert (
            "num_granularity_car" in keys and type(config["num_granularity_car"]) == int
        )
        print("valid config")
        return True
...
class Normalizer:
    def __init__(self, dfs=None, variable_length=False):
        ...
        if dfs is not None:
            ...
            res.extend(dfs)  # shape : (200,128,8)
            res = np.array(res)
    ...
    def compute_min_max(self, res):
        column_max_all = np.max(res, axis=1)  # 특징 8개마다 최댓값만 남긴 형태(200,8)
